...
export function UserFormModal({ isOpen, onClose, mode = 'create', initialUser }: UserFormModalProps) {
    ...
    const steps = [
        { id: 'type', label: t('Type de compte'), disabled: mode === 'edit' },
    ...
    const renderStepContent = () => {
        if (step === 0) {
            return (
                <div>
                    <p>{t('Choisissez le type de compte à créer :')}</p>
        ...
        if (step === 4) {
            ...
            return (
                <div className="user-form-modal__summary">
                    ...
                    <p><strong>{t('Type :')}</strong> {isProfessional ? t('Professionnel') : t('Patient')}</p>
                    ...
                    <p><strong>{t('Adresse :')}</strong> {address.street} {address.city} {address.postalCode} {address.country}</p>
                    {isProfessional && (
                        <>
                            <p><strong>{t('Licence :')}</strong> {(form as any).licenseNumber}</p>
                            <p><strong>{t('Spécialité :')}</strong> {(form as any).specialty || '—'}</p>
                        </>
                    )}
                    {isPatient && (
                        <>
                            <p><strong>{t('Date de naissance :')}</strong> {(form as any).dateOfBirth || '—'}</p>
                            <p><strong>{t('Groupe sanguin :')}</strong> {(form as any).bloodType || '—'}</p>
                        </>
                    )}
                </div>
            );
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} size="large">
            <div className="user-form-modal">
                <h2>{mode === 'create' ? t('Créer un utilisateur') : t('Modifier l\u2019utilisateur')}</h2>
                ...
                <div className="user-form-modal__footer">
                    ...
                    {step === 0 ? null : step < steps.length - 1 ? (
                        ...
                        <Button variant="primary" onClick={handleSubmit} disabled={isSubmitting}>
                            {mode === 'create' ? t('Créer') : t('Enregistrer')}
                        </Button>
                    )}
                </div>
            </div>
