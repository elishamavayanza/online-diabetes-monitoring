...
export default {
    ensurePackagesExistAndAreCorrectVersion: function (featureName, method = null) {
        ...
        packageHelper.ensurePackagesExist(packageHelper.addPackagesVersionConstraint(config.packages), method || config.method);
    },
    getMissingPackageRecommendations: function (featureName) {
        const config = getFeatureConfig(featureName);
        return packageHelper.getMissingPackageRecommendations(packageHelper.addPackagesVersionConstraint(config.packages), config.method);
    },
    ...
    ensureDevServerVersionRequirements: function () {
        ...
        const outdated = devServer6Requirements
            ...
            .filter((pkg) => pkg.installed !== null && !semver.satisfies(pkg.installed, pkg.version));
        if (outdated.length === 0) {
            return;
        }
        const installCommand = packageHelper.getInstallCommand(outdated.map((pkg) => [{ name: pkg.name, version: pkg.version }]));
