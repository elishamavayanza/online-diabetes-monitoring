...
export class EntryPointsPlugin {
    ...
    apply(compiler) {
        compiler.hooks.afterEmit.tapAsync({ name: 'EntryPointsPlugin' }, (compilation, callback) => {
                ...
                for (const fileTypes of Object.values(manifest.entrypoints)) {
                    for (const assets of Object.values(fileTypes)) {
                        for (const asset of assets) {
                            ...
                            const filePath = path.resolve(this.outputPath, assetNormalized.replace(this.publicPath, ''));
            ...
            fs.writeFileSync(path.join(this.outputPath, 'entrypoints.json'), JSON.stringify(manifest, null, 2), { flag: 'w' });
            callback();
        });
    }
}
