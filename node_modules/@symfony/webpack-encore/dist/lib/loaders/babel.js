...
export default {
    async getLoaders(webpackConfig) {
        ...
        if (!(await webpackConfig.doesBabelRcFileExist())) {
            ...
            presetEnvOptions = applyOptionsCallback(webpackConfig.babelPresetEnvOptionsCallback, presetEnvOptions);
            if (presetEnvOptions.useBuiltIns !== false) {
                throw new Error('The "useBuiltIns" and "corejs" Babel options are no longer supported: Babel 8 removed them from @babel/preset-env. Use babel-plugin-polyfill-corejs3 instead, added via Encore.configureBabel((babelConfig) => babelConfig.plugins.push(...)) or an external Babel configuration file. See https://babeljs.io/docs/v8-migration.');
            }
